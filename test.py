...
from engine.downloads import VideoDownloader, postProcessor
import youtube_dl
import requests
import os
...
class Ui_Abyss(object):
    ...
    def download_clicker(self):
        try:
            ...
            url = self.input_text.toPlainText()
            self.stat = postProcessor(url)
            self.stat.Size.connect(self.size.setText)
            self.stat.Title.connect(self.title.setText)
            self.worker = VideoDownloader()
            self.thread = QThread()
            self.thread.start()
            self.worker.moveToThread(self.thread)
            self.worker.progressChanged.connect(self.progressBar.setValue)
            self.thread.started.connect(self.worker.download(url))
